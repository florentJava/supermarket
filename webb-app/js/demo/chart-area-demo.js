...
var montant = [0, 10000, 5000, 15000, 10000, 20000, 15000, 25000, 20000, 30000, 25000, 40000];
...
.then((result) => {
  ...
  for (let index = 0; index < mois.length; index++) {   
    if(index <= date.getMonth()){
      var total = totalMount(result , index)
      mois_fig.push(mois[index])
      montant_fig.push(total)
    }
    
    
  }

  set(mois_fig,montant_fig)
 })
  ...
  function totalMount(factures,mois){
    ...
    factures.forEach(element =>{
      ...
      if(mois == date_facture.getMonth() && date_facture.getFullYear() == date.getFullYear()){
